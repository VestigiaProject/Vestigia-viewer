...
export async function fetchPost(id: string): Promise<HistoricalPostWithFigure | null> {
  try {
    const { data, error } = await supabase
      .from('historical_posts')
      .select(`
...
export async function fetchAllPostIds() {
  try {
    ...
    handleError(error, {
      userMessage: 'Failed to fetch post IDs',
      context: {}
    });
    return [];
  }
...
export async function fetchPostInteractions(postId: string, userId?: string): Promise<PostInteractions> {
  try {
    ...
    handleError(error, {
      userMessage: 'Failed to fetch post interactions',
      context: { 
        postId,
        userId
      }
